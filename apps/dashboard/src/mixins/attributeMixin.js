...
export default {
  computed: {
    ...
    contactIdentifier() {
      return this.currentChat.meta?.sender?.id || this.$route.params.contactId || this.contactId;
    },
    contact() {
    ...
    filteredAttributes() {
      return Object.keys(this.customAttributes).map(key => {
        const item = this.attributes.find(attribute => attribute.attribute_key === key);
        if (item) {
          return {
            ...item,
        ...
        return {
          ...
          attribute_display_type: this.attributeDisplayType(this.customAttributes[key]),
          attribute_key: key,
