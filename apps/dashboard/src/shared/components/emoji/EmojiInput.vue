  ...
  <div role="dialog"
    ...
    <div class="flex flex-col">
      <div class="emoji-search--wrap flex gap-2">
        <input ref="searchbar" v-model="search" type="text"
          class="emoji-search--input focus:box-shadow-blue dark:focus:box-shadow-dark"
          :placeholder="$t('EMOJI.PLACEHOLDER')" />
        <woot-button v-if="showRemoveButton" size="small" variant="smooth"
          class="dark:!bg-slate-800 dark:!hover:bg-slate-700" color-scheme="secondary" @click="onClick('')">
          {{ $t('EMOJI.REMOVE') }}
        </woot-button>
      </div>
      <div v-if="hasNoSearch" ref="emojiItem" class="emoji-item">
        ...
        <div class="emoji--row">
          <button v-for="item in filterEmojisByCategory" :key="item.slug" v-dompurify-html="item.emoji"
            class="emoji--item" track-by="$index" @click="onClick(item.emoji)" />
        </div>
      </div>
      <div v-else ref="emojiItem" class="emoji-item">
        <div v-for="category in filterAllEmojisBySearch" :key="category.slug">
          ...
          <div v-if="category.emojis.length > 0" class="emoji--row">
            <button v-for="item in category.emojis" :key="item.slug" v-dompurify-html="item.emoji" class="emoji--item"
              track-by="$index" @click="onClick(item.emoji)" />
          </div>
        </div>
      ...
      <div class="emoji-dialog--footer" role="menu">
        <ul>
          <li>
            <button class="emoji--item" :class="{ active: selectedKey === 'Search' }" @click="changeCategory('Search')">
              <fluent-icon icon="search" size="16" class="text-slate-700 dark:text-slate-100" />
            </button>
          </li>
          <li v-for="category in categories" :key="category.slug" @click="changeCategory(category.name)">
            <button v-dompurify-html="getFirstEmojiByCategoryName(category.name)" class="emoji--item"
              :class="{ active: selectedKey === category.name }" @click="changeCategory(category.name)" />
          </li>
        </ul>
      </div>
    </div>
