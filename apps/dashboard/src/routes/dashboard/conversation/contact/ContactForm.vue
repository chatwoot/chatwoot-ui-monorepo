<template>
  <form class="contact--form w-full pt-6 px-8 pb-8" @submit.prevent="handleSubmit">
    <div>
      <div class="w-full">
    ...
    <woot-input v-model.trim="companyName" class="w-full" :label="$t('CONTACT_FORM.FORM.COMPANY_NAME.LABEL')"
    ...
    <div>
      <div class="w-full">
        ...
        <multiselect v-model="country" track-by="id" label="name"
          ...
          :deselect-label="$t('CONTACT_FORM.FORM.COUNTRY.REMOVE')" :custom-label="countryNameWithCode" :max-height="160"
          :options="countries" :allow-empty="true" :option-height="104" />
      </div>
    </div>
    ...
    <div class="w-full">
      ...
      <div v-for="socialProfile in socialProfileKeys" :key="socialProfile.key" class="input-group">
        <span class="input-group-label">{{ socialProfile.prefixURL }}</span>
        <input v-model="socialProfileUserNames[socialProfile.key]" class="input-group-field" type="text" />
      </div>
    </div>
