  ...
  <form @submit.prevent="changePassword()">
    <div class="profile--settings--row text-black-900 dark:text-slate-300 row">
      ...
      <div class="columns small-9 medium-5">
        <woot-input v-model="currentPassword" type="password" :class="{ error: v$.currentPassword.$error }"
          :label="$t('PROFILE_SETTINGS.FORM.CURRENT_PASSWORD.LABEL')" :placeholder="$t('PROFILE_SETTINGS.FORM.CURRENT_PASSWORD.PLACEHOLDER')
            " :error="v$.currentPassword.$error
    ? $t('PROFILE_SETTINGS.FORM.CURRENT_PASSWORD.ERROR')
    : ''
    " @blur="v$.currentPassword.$touch" />

        <woot-input v-model="password" type="password" :class="{ error: v$.password.$error }"
          :label="$t('PROFILE_SETTINGS.FORM.PASSWORD.LABEL')"
          :placeholder="$t('PROFILE_SETTINGS.FORM.PASSWORD.PLACEHOLDER')" :error="v$.password.$error ? $t('PROFILE_SETTINGS.FORM.PASSWORD.ERROR') : ''
            " @blur="v$.password.$touch" />

        <woot-input v-model="passwordConfirmation" type="password" :class="{ error: v$.passwordConfirmation.$error }"
          :label="$t('PROFILE_SETTINGS.FORM.PASSWORD_CONFIRMATION.LABEL')" :placeholder="$t('PROFILE_SETTINGS.FORM.PASSWORD_CONFIRMATION.PLACEHOLDER')
            " :error="v$.passwordConfirmation.$error
    ? $t('PROFILE_SETTINGS.FORM.PASSWORD_CONFIRMATION.ERROR')
    : ''
    " @blur="v$.passwordConfirmation.$touch" />

        <woot-button :is-loading="isPasswordChanging" type="submit" :disabled="!currentPassword ||
