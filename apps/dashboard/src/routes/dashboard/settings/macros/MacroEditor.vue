<template>
  <div class="h-full overflow-auto flex-1 flex flex-col">
    <woot-loading-state v-if="uiFlags.isFetchingItem" :message="$t('MACROS.EDITOR.LOADING')" />
    <macro-form v-if="macro && !uiFlags.isFetchingItem" v-model:macro-data="macro" @submit="saveMacro" />
  </div>
</template>
