<template>
  <div class="flex flex-col md:flex-row h-auto md:h-full w-full">
    <div
      ...
      <macro-nodes v-model="macro.actions" :files="files" @addNewNode="appendNode" @deleteNode="deleteNode"
        @resetAction="resetNode" />
    </div>
    <div class="w-full md:w-1/3">
      <macro-properties :macro-name="macro.name" :macro-visibility="macro.visibility" @update:name="updateName"
        @update:visibility="updateVisibility" @submit="submit" />
    </div>
  </div>
</template>
