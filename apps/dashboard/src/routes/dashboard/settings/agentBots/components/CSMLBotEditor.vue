    ...
    <div class="flex flex-row">
      <div class="w-[68%]">
        <div class="h-[calc(100vh-56px)] relative">
          ...
          <div v-if="v$.bot.csmlContent.$error"
        ...
        <form class="flex flex-col justify-between h-full" @submit.prevent="onSubmit">
          <div>
            <label :class="{ error: v$.bot.name.$error }">
              ...
              <input v-model="bot.name" type="text" :placeholder="$t('AGENT_BOTS.CSML_BOT_EDITOR.NAME.PLACEHOLDER')" />
            ...
            <label>
              ...
              <textarea v-model="bot.description" rows="4" :placeholder="$t('AGENT_BOTS.CSML_BOT_EDITOR.DESCRIPTION.PLACEHOLDER')
                " />
            </label>
            <woot-button>
