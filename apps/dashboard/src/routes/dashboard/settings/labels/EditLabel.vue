  ...
  <div class="h-auto overflow-auto flex flex-col">
    ...
    <form class="mx-0 flex flex-wrap" @submit.prevent="editLabel">
      <woot-input v-model.trim="title" :class="{ error: v$.title.$error }" class="w-full label-name--input"
        ...
        :error="getLabelTitleErrorMessage" @input="v$.title.$touch" />
      ...
      <div class="flex justify-end items-center py-2 px-0 gap-2 w-full">
        <woot-button :is-disabled="v$.title.$invalid || uiFlags.isUpdating" :is-loading="uiFlags.isUpdating">
...
export default {
  ...
  computed: {
    ...
    pageTitle() {
      return `${this.$t('LABEL_MGMT.EDIT.TITLE')} - ${this.selectedResponse.title
        }`;
    },
  },
  mounted() {
