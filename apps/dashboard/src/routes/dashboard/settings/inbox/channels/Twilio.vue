  ...
  <form class="mx-0 flex flex-wrap" @submit.prevent="createChannel()">
    <div class="w-[65%] flex-shrink-0 flex-grow-0 max-w-[65%]">
      <label :class="{ error: v$.channelName.$error }">
        ...
        <input v-model.trim="channelName" type="text" :placeholder="$t('INBOX_MGMT.ADD.TWILIO.CHANNEL_NAME.PLACEHOLDER')"
    ...
    <div class="w-[65%] flex-shrink-0 flex-grow-0 max-w-[65%]">
      <label v-if="useMessagingService" :class="{ error: v$.messagingServiceSID.$error }">
        {{ $t('INBOX_MGMT.ADD.TWILIO.MESSAGING_SERVICE_SID.LABEL') }}
        <input v-model.trim="messagingServiceSID" type="text" :placeholder="$t('INBOX_MGMT.ADD.TWILIO.MESSAGING_SERVICE_SID.PLACEHOLDER')
    ...
    <div v-if="!useMessagingService" class="w-[65%] flex-shrink-0 flex-grow-0 max-w-[65%]">
      <label :class="{ error: v$.phoneNumber.$error }">
        {{ $t('INBOX_MGMT.ADD.TWILIO.PHONE_NUMBER.LABEL') }}
        <input v-model.trim="phoneNumber" type="text" :placeholder="$t('INBOX_MGMT.ADD.TWILIO.PHONE_NUMBER.PLACEHOLDER')"
    ...
    <div class="medium-8 columns messagingServiceHelptext">
      <label for="useMessagingService">
        <input id="useMessagingService" v-model="useMessagingService" type="checkbox" class="checkbox" />
        {{
          $t(
    ...
    <div class="w-[65%] flex-shrink-0 flex-grow-0 max-w-[65%]">
      <label :class="{ error: v$.accountSID.$error }">
        ...
        <input v-model.trim="accountSID" type="text" :placeholder="$t('INBOX_MGMT.ADD.TWILIO.ACCOUNT_SID.PLACEHOLDER')"
    ...
    <div class="medium-8 columns messagingServiceHelptext">
      <label for="useAPIKey">
        <input id="useAPIKey" v-model="useAPIKey" type="checkbox" class="checkbox" />
    ...
    <div v-if="useAPIKey" class="w-[65%] flex-shrink-0 flex-grow-0 max-w-[65%]">
      <label :class="{ error: v$.apiKeySID.$error }">
        ...
        <input v-model.trim="apiKeySID" type="text" :placeholder="$t('INBOX_MGMT.ADD.TWILIO.API_KEY.PLACEHOLDER')"
    ...
    <div class="w-[65%] flex-shrink-0 flex-grow-0 max-w-[65%]">
      <label :class="{ error: v$.authToken.$error }">
        ...
        <input v-model.trim="authToken" type="text" :placeholder="$t(`INBOX_MGMT.ADD.TWILIO.${authTokeni18nKey}.PLACEHOLDER`)
    ...
    <div class="w-full">
      <woot-submit-button :loading="uiFlags.isCreating" :button-text="$t('INBOX_MGMT.ADD.TWILIO.SUBMIT_BUTTON')" />
    </div>
  </form>
</template>
