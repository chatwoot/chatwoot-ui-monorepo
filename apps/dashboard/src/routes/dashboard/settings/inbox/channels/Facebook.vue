  ...
  <div class="wizard-body w-[75%] flex-shrink-0 flex-grow-0 max-w-[75%]">
    ...
    <div v-else>
      ...
      <form v-else class="mx-0 flex flex-wrap" @submit.prevent="createChannel()">
        <div class="w-full">
          <page-header :header-title="$t('INBOX_MGMT.ADD.DETAILS.TITLE')" :header-content="useInstallationName(
        ...
        <div class="w-[60%]">
          <div class="w-full">
            <div class="input-wrap" :class="{ error: v$.selectedPage.$error }">
              ...
              <multiselect v-model.trim="selectedPage" :close-on-select="true" :allow-empty="true"
                :options="getSelectablePages" track-by="id" label="name"
                :select-label="$t('FORMS.MULTISELECT.ENTER_TO_SELECT')"
                :deselect-label="$t('FORMS.MULTISELECT.ENTER_TO_REMOVE')"
                :placeholder="$t('INBOX_MGMT.ADD.FB.PICK_A_VALUE')" selected-label @select="setPageName" />
          ...
          <div class="w-full">
            <label :class="{ error: v$.pageName.$error }">
              ...
              <input v-model.trim="pageName" type="text" :placeholder="$t('INBOX_MGMT.ADD.FB.PICK_NAME')"
