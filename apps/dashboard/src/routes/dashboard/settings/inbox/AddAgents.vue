<template>
  <div class="wizard-body w-[75%] flex-shrink-0 flex-grow-0 max-w-[75%]">
    <form class="mx-0 flex flex-wrap" @submit.prevent="addAgents()">
      <div class="w-full">
        <page-header :header-title="$t('INBOX_MGMT.ADD.AGENTS.TITLE')"
      ...
      <div class="w-[60%]">
        <div class="w-full">
          <label :class="{ error: v$.selectedAgents.$error }">
            ...
            <multiselect v-model="selectedAgents" :options="agentList" track-by="id" label="name" :multiple="true"
              :close-on-select="false" :clear-on-select="false" :hide-selected="true" selected-label
              :select-label="$t('FORMS.MULTISELECT.ENTER_TO_SELECT')"
              :deselect-label="$t('FORMS.MULTISELECT.ENTER_TO_REMOVE')"
              :placeholder="$t('INBOX_MGMT.ADD.AGENTS.PICK_AGENTS')" @select="v$.selectedAgents.$touch" />
        ...
        <div class="w-full">
          <woot-submit-button :button-text="$t('INBOX_MGMT.AGENTS.BUTTON_TEXT')" :loading="isCreating" />
        </div>
      </div>
    </form>
  </div>
