<template>
  <div class="flex-1 overflow-auto p-4">
    ...
    <div class="flex flex-row gap-4">
      <div class="w-[60%]">
        <p v-if="!inboxesList.length" class="flex h-full items-center flex-col justify-center">
          {{ $t('INBOX_MGMT.LIST.404') }}
          <router-link v-if="isAdmin" :to="addAccountScoping('settings/inboxes/new')">
        ...
        <table v-if="inboxesList.length" class="woot-table">
          <tbody>
            <tr v-for="item in inboxesList" :key="item.id">
              <td>
                <img v-if="item.avatar_url" class="woot-thumbnail" :src="item.avatar_url" alt="No Page Image" />
                <img v-else class="woot-thumbnail" src="~dashboard/assets/images/flag.svg" alt="No Page Image" />
              </td>
              <!-- Short Code  -->
              <td>
              ...
              <td>
                <div class="button-wrapper">
                  ...
                  <woot-button v-if="isAdmin" v-tooltip.top="$t('INBOX_MGMT.DELETE.BUTTON_TEXT')" variant="smooth"
                    color-scheme="alert" size="tiny" class-names="grey-btn" :is-loading="loading[item.id]"
                    icon="dismiss-circle" @click="openDelete(item)" />
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div class="w-[34%]">
        <span v-dompurify-html="useInstallationName(
    ...
    <settings v-if="showSettings" v-model:show="showSettings" :on-close="closeSettings" :inbox="selectedInbox" />

    <woot-confirm-delete-modal v-if="showDeletePopup" v-model:show="showDeletePopup"
      :title="$t('INBOX_MGMT.DELETE.CONFIRM.TITLE')" :message="confirmDeleteMessage" :confirm-text="deleteConfirmText"
      :reject-text="deleteRejectText" :confirm-value="selectedInbox.name"
...
export default {
  ...
  computed: {
    ...
    // Delete Modal
    deleteConfirmText() {
      return `${this.$t('INBOX_MGMT.DELETE.CONFIRM.YES')} ${this.selectedInbox.name
        }`;
    },
    deleteRejectText() {
      return `${this.$t('INBOX_MGMT.DELETE.CONFIRM.NO')} ${this.selectedInbox.name
        }`;
    },
    confirmDeleteMessage() {
      return `${this.$t('INBOX_MGMT.DELETE.CONFIRM.MESSAGE')} ${this.selectedInbox.name
        }?`;
    },
