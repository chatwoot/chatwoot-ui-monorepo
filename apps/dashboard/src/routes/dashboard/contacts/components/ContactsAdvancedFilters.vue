    ...
    <div class="p-8">
      <div v-if="isSegmentsView">
        <label class="input-label" :class="{ error: !activeSegmentNewName }">
          ...
          <input v-model="activeSegmentNewName" type="text"
      ...
      <div
        ...
        <filter-input-box v-for="(filter, i) in appliedFilters" :key="i" v-model="appliedFilters[i]"
          :filter-groups="filterGroups" :grouped-filters="true" :input-type="getInputType(
          ...
          :show-user-input="showUserInput(appliedFilters[i].filter_operator)" :v="v$.appliedFilters.$each[i]"
          @resetFilter="resetFilter(i, appliedFilters[i])" @removeFilter="removeFilter(i)" />
        <div class="mt-4">
          <woot-button icon="add" color-scheme="success" variant="smooth" size="small" @click="appendNewFilter">
            {{ $t('CONTACTS_FILTER.ADD_NEW_FILTER') }}
          </woot-button>
          <woot-button v-if="hasAppliedFilters && !isSegmentsView" icon="subtract" color-scheme="alert" variant="smooth"
      ...
      <div class="w-full">
        <div class="flex flex-row justify-end gap-2 py-2 px-0 w-full">
          ...
          <woot-button v-if="isSegmentsView" :disabled="!activeSegmentNewName" @click="updateSegment">
