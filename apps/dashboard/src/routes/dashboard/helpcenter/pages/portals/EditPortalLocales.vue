<template>
  <div class="portal-locales">
    <div class="button-container">
      <woot-button variant="smooth" size="small" color-scheme="primary" class="header-action-buttons" @click="addLocale">
        {{ $t('HELP_CENTER.PORTAL.PORTAL_SETTINGS.LIST_ITEM.HEADER.ADD') }}
      </woot-button>
    </div>
    <div class="locale-container">
      <locale-item-table :locales="locales" :selected-locale-code="currentPortal.meta.default_locale"
        @change-default-locale="changeDefaultLocale" @delete="deletePortalLocale" />
    </div>
    <woot-modal v-model:show="isAddLocaleModalOpen" :on-close="closeAddLocaleModal">
      <add-locale :show="isAddLocaleModalOpen" :portal="currentPortal" @cancel="closeAddLocaleModal" />
    </woot-modal>
  </div>
</template>
<script>
