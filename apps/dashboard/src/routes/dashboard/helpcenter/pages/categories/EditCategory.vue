...
<template>
  <woot-modal :show="show" @update:show="$emit('update:show', $event)" :on-close="onClose">
    <woot-modal-header :header-title="$t('HELP_CENTER.CATEGORY.EDIT.TITLE')"
    ...
    <form class="w-full" @submit.prevent="onUpdate">
      <div class="w-full">
        ...
        <category-name-icon-input :label="$t('HELP_CENTER.CATEGORY.EDIT.NAME.LABEL')"
          :placeholder="$t('HELP_CENTER.CATEGORY.EDIT.NAME.PLACEHOLDER')"
          :help-text="$t('HELP_CENTER.CATEGORY.EDIT.NAME.HELP_TEXT')" :has-error="v$.name.$error"
          :error-message="$t('HELP_CENTER.CATEGORY.ADD.NAME.ERROR')" :existing-name="category.name"
          :saved-icon="category.icon" @name-change="changeName" @icon-change="onClickInsertEmoji" />
        <woot-input v-model.trim="slug" :class="{ error: v$.slug.$error }" class="w-full" :error="slugError"
          :label="$t('HELP_CENTER.CATEGORY.EDIT.SLUG.LABEL')"
          :placeholder="$t('HELP_CENTER.CATEGORY.EDIT.SLUG.PLACEHOLDER')"
          :help-text="$t('HELP_CENTER.CATEGORY.EDIT.SLUG.HELP_TEXT')" @input="v$.slug.$touch" />
        <label>
          {{ $t('HELP_CENTER.CATEGORY.EDIT.DESCRIPTION.LABEL') }}
          <textarea v-model="description" rows="3" type="text" :placeholder="$t('HELP_CENTER.CATEGORY.EDIT.DESCRIPTION.PLACEHOLDER')
            " />
        </label>
        <div class="w-full">
