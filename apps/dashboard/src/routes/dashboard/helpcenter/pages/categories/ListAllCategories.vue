  ...
  <div class="w-full pl-4">
    <header class="flex justify-between items-center mb-4">
      <div class="flex items-center w-full gap-3">
        <label class="font-normal mb-0 text-base text-slate-800 dark:text-slate-100">
          {{ $t('HELP_CENTER.PORTAL.EDIT.CATEGORIES.TITLE') }}
        </label>
        <select :value="currentLocaleCode" class="w-[15%] select-locale" @change="changeCurrentCategory">
          <option v-for="allowedLocaleCode in allowedLocaleCodes" :key="allowedLocaleCode" :value="allowedLocaleCode">
            {{ allowedLocaleCode }}
          </option>
        </select>
      </div>
      <div class="flex-none items-center">
        <woot-button size="small" variant="smooth" color-scheme="primary" icon="add" @click="openAddCategoryModal">
          {{ $t('HELP_CENTER.PORTAL.EDIT.CATEGORIES.NEW_CATEGORY') }}
        </woot-button>
      </div>
    </header>
    <div class="category-list">
      <category-list-item :categories="categoriesByLocaleCode" @delete="deleteCategory" @edit="openEditCategoryModal" />
    </div>
    <edit-category v-if="showEditCategoryModal" v-model:show="showEditCategoryModal" :portal-name="currentPortalName"
      :locale="selectedCategory.locale" :category="selectedCategory" :selected-portal-slug="currentPortalSlug"
      @cancel="closeEditCategoryModal" />
    <add-category v-if="showAddCategoryModal" v-model:show="showAddCategoryModal" :portal-name="currentPortalName"
      :locale="currentLocaleCode" @cancel="closeAddCategoryModal" />
  </div>
</template>
<script>
