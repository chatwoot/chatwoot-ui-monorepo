...
<template>
  <woot-modal :show="show" @update:show="$emit('update:show', $event)" :on-close="onClose">
    <woot-modal-header :header-title="$t('HELP_CENTER.CATEGORY.ADD.TITLE')"
    ...
    <form class="w-full" @submit.prevent="onCreate">
      <div class="w-full">
        ...
        <category-name-icon-input :label="$t('HELP_CENTER.CATEGORY.ADD.NAME.LABEL')"
          :placeholder="$t('HELP_CENTER.CATEGORY.ADD.NAME.PLACEHOLDER')"
          :help-text="$t('HELP_CENTER.CATEGORY.ADD.NAME.HELP_TEXT')" :has-error="v$.name.$error"
          :error-message="$t('HELP_CENTER.CATEGORY.ADD.NAME.ERROR')" @name-change="onNameChange"
        ...
        <woot-input v-model.trim="slug" :class="{ error: v$.slug.$error }" class="w-full" :error="slugError"
          ...
          :help-text="$t('HELP_CENTER.CATEGORY.ADD.SLUG.HELP_TEXT')" @input="v$.slug.$touch" />
        <label>
          {{ $t('HELP_CENTER.CATEGORY.ADD.DESCRIPTION.LABEL') }}
          <textarea v-model="description" rows="3" type="text" :placeholder="$t('HELP_CENTER.CATEGORY.ADD.DESCRIPTION.PLACEHOLDER')
            " />
        </label>
        <div class="w-full">
