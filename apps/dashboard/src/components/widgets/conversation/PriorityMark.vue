<template>
  <span v-if="priority" v-tooltip="{
    content: tooltipText,
    delay: { show: 1500, hide: 0 },
    hideOnClick: true,
  }" class="shrink-0 rounded-sm inline-flex w-3.5 h-3.5" :class="{
  ...
  'bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-200':
    ...
    <fluent-icon :icon="`priority-${priority.toLowerCase()}`" size="14" view-box="0 0 14 14" />
  </span>
</template>
