<template>
  <div class="message-text__wrap" :class="attachmentTypeClasses">
    <img v-if="isImage && !isImageError" :src="attachment.data_url" @click="onClick" @error="onImgError" />
    <video v-if="isVideo" :src="attachment.data_url" muted playsInline @error="onImgError" @click="onClick" />
    <audio v-else-if="isAudio" controls class="skip-context-menu">
      <source :src="`${attachment.data_url}?t=${Date.now()}`" />
    </audio>
    <gallery-view v-if="show" :show="show" @update:show="$emit('update:show', $event)" :attachment="attachment"
      :all-attachments="filteredCurrentChatAttachments" @error="onImgError" @close="onClose" />
  </div>
</template>
